...
- Freshness distinguishes a series between prints from one whose next print is late. Age alone cannot: M2 at 38 days is on schedule while NFCI at 12 is overdue, and showing both as an age made a data outage look like a quiet month. `describeSeriesFreshness` carries each series' normal publication gap alongside its staleness tolerance and reports current, overdue, or stale — an overdue series is flagged in the observation strip while still counting toward the models, since it is late rather than unusable.
