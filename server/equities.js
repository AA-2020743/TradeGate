...
export async function getEquityDashboard(requestedSymbol = 'SPY') {
  ...
  // change it.
  // Built from the regime model's own drivers rather than a second set of
  // weights, so the banner and the panel beneath it cannot disagree.
  const verdict = buildVerdictFromModel(regime, {
    title: `${index.name} regime`,
    version: 'equity-verdict-v1',
    meaning: { high: 'supportive of risk', low: 'restrictive' },
    details: {
      trend: usableTechnical?.components?.trendAveragesUsed ? `${usableTechnical.components.trendAveragesUsed} of 3 moving averages` : null,
      momentum: Number.isFinite(usableTechnical?.indicators?.momentum20d) ? `${usableTechnical.indicators.momentum20d.toFixed(1)}% over 20 sessions` : null,
      breadth: Number.isFinite(constituentBreadth?.pctAbove200) ? `${constituentBreadth.pctAbove200}% above the 200-day` : null,
      liquidity: liquidity?.model?.regime ?? null,
      volatility: Number.isFinite(usableTechnical?.indicators?.annualizedVolatility20d) ? `${usableTechnical.indicators.annualizedVolatility20d.toFixed(1)}% annualized` : null,
    },
    reasons: {
      breadth: constituentBreadth?.reason ?? 'Constituent histories are not connected',
      liquidity: liquidity?.model?.reason ?? 'The liquidity model did not publish',
    },
  });
  ...
  const dashboardVintage = resolveVintage([
