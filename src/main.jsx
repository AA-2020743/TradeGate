 ...
 */
/**
 * A driver's change, annotated when the window it actually covered is not the
 * window the column header claims. A monthly series answering a 91-day
 * question lands on the observation nearest 91 days back, which can be 120 -
 * and that change sits in the same weighted composite as two genuine 91-day
 * ones, so the difference belongs on the row.
 */
function DriverChange({ driver }) {
  if (!Number.isFinite(driver.changePercent)) return <strong>—</strong>;
  const drifted = Number.isFinite(driver.spanDays) && Number.isFinite(driver.requestedDays)
    && Math.abs(driver.spanDays - driver.requestedDays) > driver.requestedDays * 0.1;
  return <strong>
    {`${driver.changePercent >= 0 ? '+' : ''}${driver.changePercent.toFixed(2)}%`}
    {drifted ? <small className="driver-span"> over {driver.spanDays}d</small> : null}
  </strong>;
}

/**
 * A section kicker whose status word comes from the panel's own model.
 *
 * These were hardcoded, so eighteen panels announced "CALCULATED" while their
 * bodies explained they had nothing - the same contradiction reported on the
  ...
  return <div className="equities-dashboard">
    ...
    <section className="equity-index-layout">
      ...
      <article className="equity-focus-panel panel">
        ...
        {dashboard?.technical?.reason
          ? <p className="equity-source-line equity-technical-reason">Nothing to score: {dashboard.technical.reason}.</p>
          : null}
        <p className="equity-source-line">{dashboard?.technical?.usedSource ?? dashboard?.technical?.source ?? 'No market history source'} · {formatResearchDate(dashboard?.technical?.asOf)}</p>
      </article>
    </section>
    ...
    <section className="equity-regime-layout">
      <article className="equity-driver-panel panel"><div className="panel-title"><div><p className="section-kicker">REGIME INPUTS</p><h3>{regime?.regime ?? 'Waiting for minimum coverage'}</h3></div><EquityStatus status={regime?.status} label={Number.isFinite(regime?.coverage) ? `${regime.coverage}% coverage` : 'unavailable'} /></div><div className="equity-driver-list">{(regime?.drivers ?? []).map((driver) => <div key={driver.key}><span><b>{driver.name}</b><small>{driver.source ?? 'Source unavailable'}</small></span><i><b style={{ width: `${driver.score ?? 0}%` }}></b></i><strong>{driver.score ?? '—'}</strong></div>)}</div></article>
    ...
    <section className="equity-section-heading"><div><StatusKicker label="STYLE AND REGION ROTATION" published={sectorData?.styles?.status === 'calculated'} /><h2>Basket spreads across the cycle</h2></div><EquityStatus status={sectorData?.styles?.status} /></section>
    <section className="equity-macro-matrix">
      <article className="equity-rotation-panel panel wide">
    ...
    <section className="equity-macro-matrix">
      ...
      {(() => {
        ...
        return <article className={`equity-rotation-panel panel wide ${hasSentiment ? '' : 'preview-section'}`}>
          <div className="panel-title"><div><StatusKicker label="MARKET SENTIMENT" published={hasSentiment} /><h3>CNN Fear &amp; Greed composite</h3></div><span className="data-pill">{hasSentiment ? fearGreed.rating : 'Unconfigured'}</span></div>
          {hasSentiment ? <>
    ...
    <section className="equity-macro-matrix">
      {(() => {
        ...
        return <article className={`equity-rotation-panel panel wide ${hasRisk ? '' : 'preview-section'}`}>
          <div className="panel-title"><div><p className="section-kicker">EQUITY RISK DASHBOARD · {risk?.version ? risk.version.toUpperCase() : 'UNAVAILABLE'}</p><h3>{risk?.spxBreadth?.divergence?.divergent ? `${risk.spxBreadth.divergence.state}: ${risk.spxBreadth.pctAbove200}% of S&P 500 above 200-day` : risk?.spxBreadth?.status === 'calculated' ? `${risk.spxBreadth.pctAbove200}% of S&P 500 above 200-day — ${risk.spxBreadth.read}` : 'Breadth and risk appetite'}</h3></div><span className="data-pill">{Number.isFinite(risk?.calculatedCount) && Number.isFinite(risk?.totalLegs) ? `${risk.calculatedCount}/${risk.totalLegs} legs` : 'Unavailable'}</span></div>
          {hasRisk ? <>
...
function MetalsDashboard({ data }) {
  ...
  return <div className="metals-dashboard">
    ...
    <section className="metals-bottom-grid">
      <article className={`miners-panel panel ${(workspace?.miners ?? []).length ? '' : 'preview-section'}`}><div className="panel-title"><div><StatusKicker label="MINER EQUITY EXPRESSION" published={Boolean((workspace?.miners ?? []).length)} /><h3>Miner ETF momentum</h3></div><span className="data-pill">20-session</span></div><div className="miner-list">{(workspace?.miners ?? []).map((miner) => <button key={miner.symbol}><span>{miner.symbol}</span><b>{miner.name}</b><small className={(miner.change20d ?? 0) >= 0 ? 'positive' : 'negative'}>{Number.isFinite(miner.change20d) ? `${miner.change20d > 0 ? '+' : ''}${miner.change20d}%` : '—'}</small><i>↗</i></button>)}{!(workspace?.miners ?? []).length && <div className="equity-empty">Miner histories are required before momentum can publish.</div>}</div><p>Miners add operating leverage to metal prices, but input costs and equity-beta remain separate risks.</p></article>
...
function IntradayRotationPanel({ fallback }) {
  ...
  return <article className={`crypto-tailwind-panel panel ${calculated ? '' : 'preview-section'}`}>
    <div className="panel-title">
      <div>
        <StatusKicker label="INTRADAY ROTATION TIMING" published={calculated} />
...
function ForexDashboard({ data }) {
  ...
  // The kicker used to be hardcoded "CALCULATED", so the panel announced a
  // calculation while its body explained it had none. And "contracts are
  // required" describes the symptom, not the cause: a rejected app token, a
  // throttled anonymous pool and a genuine outage each need a different fix.
  const cotPublished = Boolean(fxWorkspace?.pairs?.some((pair) => pair.cot) || fxWorkspace?.usdCot);
  const cotProvider = fxWorkspace?.positioningProvider ?? null;
  const cotReason = cotProvider?.note
    ?? (fxWorkspace?.positioningErrors?.length
      ? `CFTC did not return contracts: ${fxWorkspace.positioningErrors[0]}`
      : 'CFTC currency contracts are required before positioning can publish.');
...
function CryptoDashboard({ data }) {
  ...
  return <div className="crypto-dashboard">
    ...
    <section className="crypto-grid">
      ...
      <article className={`crypto-tailwind-panel panel ${data.bitcoin?.stablecoinLead?.status === 'calculated' ? '' : 'preview-section'}`}><div className="panel-title"><div><StatusKicker label="STABLECOIN ↔ BTC LEAD/LAG" published={Boolean(data.bitcoin?.stablecoinLead?.read)} /><h3>{data.bitcoin?.stablecoinLead?.read ?? 'Awaiting aligned histories'}</h3></div><span className="data-pill">{data.bitcoin?.stablecoinLead?.version ?? 'Unavailable'}</span></div>
        <div className="btc-cycle-grid">
      ...
      <article className={`dxy-btc-panel panel ${hasBtc ? '' : 'preview-section'}`}><div className="panel-title"><div><StatusKicker label="BITCOIN CYCLE & CROWDING" published={hasBtc} /><h3>{btc?.trend?.status === 'calculated' ? `Spot $${Math.round(btc.trend.price).toLocaleString()} · ${btc.trend.pctVsSma200w > 0 ? '+' : ''}${btc.trend.pctVsSma200w}% vs 200W` : 'Cycle dashboard'}</h3></div><span className="data-pill">{Number.isFinite(btc?.calculatedCount) && Number.isFinite(btc?.totalLegs) ? `${btc.calculatedCount}/${btc.totalLegs} legs` : 'Unavailable'}</span></div>
        {hasBtc ? <>
          <div className="btc-cycle-grid">
