...
function CryptoDashboard({ data }) {
  ...
  return <div className="crypto-dashboard">
    ...
    <section className="crypto-grid">
      ...
      <article className={`dxy-btc-panel panel ${hasBtc ? '' : 'preview-section'}`}><div className="panel-title"><div><p className="section-kicker">BITCOIN CYCLE &amp; CROWDING · CALCULATED</p><h3>{btc?.trend?.status === 'calculated' ? `Spot $${Math.round(btc.trend.price).toLocaleString()} · ${btc.trend.pctVsSma200w > 0 ? '+' : ''}${btc.trend.pctVsSma200w}% vs 200W` : 'Cycle dashboard'}</h3></div><span className="data-pill">{Number.isFinite(btc?.calculatedCount) && Number.isFinite(btc?.totalLegs) ? `${btc.calculatedCount}/${btc.totalLegs} legs` : 'Unavailable'}</span></div>
        {hasBtc ? <>
          <div className="btc-cycle-grid">
